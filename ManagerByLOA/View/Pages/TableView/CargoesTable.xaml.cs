...
{
	/// <summary>
	/// Страница с таблицей [Грузов в ТМЦ]
	/// </summary>
	/// <inheritdoc cref="Page" />
	public partial class CargoesTable : IPageable
	{
		/// <summary>
		...
		public CargoesTable()
		{
			...
			PageUtil.SelectSpecifiedOrFirstDataGridRow<Cargo>(PageDataGrid, null);
		}
		
		public CargoesTable(Cargo selected)
		{
			InitializeComponent();
			AdditionalInitializeComponent();
			VisualInitializeComponent();
		...
		private void EditButton_OnClick(object senderIsButton, RoutedEventArgs eventArgs)
		{
			...
			var cargo = frameworkElement.DataContext as Cargo;
			if (cargo == null)
			{
				return;
			}
			PageSwitcher.Switch(new CargoEdit(cargo));
		}
		...
		private void DeleteButton_OnClick(object senderIsButton, RoutedEventArgs eventArgs)
		{
			...
			DeleteEntityIfConfirmation(cargo); // Удаление сущности, если пользователь подтвердит намерения
		}
		...
		private void PageDataGrid_OnPreviewKeyDown(object senderIsDataGrid, KeyEventArgs eventArgs)
		{
			...
			var selected = dataGrid.SelectedItem as Cargo;
			if (selected == null)
			{
				return;
			}
			...
			{
				eventArgs.Handled = true;
				PageSwitcher.Switch(new CargoEdit(selected));
			}

			// Если нажата [Delete] и текущее значение сервисного поля не противоречит удалению
		...
		/// </summary>
		private static void DeleteEntityIfConfirmation(Cargo cargo)
		{
		...
		private bool MapFilterPredicate(object rawEntity)
		{
			...
			result &= !filter.GetValue("Name", out buffer) || FilterCriterias.ContainsLine(cargo.Name, buffer);
			result &= !filter.GetValue("Measure.Name", out buffer) ||
			          FilterCriterias.ContainsLine(cargo.Measure != null ? cargo.Measure.Name : string.Empty, buffer);
			result &= !filter.GetValue("ServiceCountUsed", out buffer) 
			          || FilterCriterias.ContainsLong(cargo.ServiceCountUsed, buffer);
			return result;
		}
