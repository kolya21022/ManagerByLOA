...
using ManagerByLetterOfAttorney.Properties;
...
namespace ManagerByLetterOfAttorney.View.Pages.TableView
{
	...
	public partial class EmployeesTable : IPageable
	{
		/// <summary>
		...
		public EmployeesTable()
		{
			...
			PageUtil.SelectSpecifiedOrFirstDataGridRow<Employee>(PageDataGrid, null);
		}
		...
		public void AdditionalInitializeComponent()
		{
			...
			const string workMonthPattern = PageLiterals.PatternEmployeesTableWorkMonth;
			try
			{
				// В зависимости от указанного параметра, показываем уволенных сотрудников в таблице или нет
				var isGetDismissed = Settings.Default.IsShowDismissalEmployeesInTable;
				var employees = EmployeesService.GetAll(isGetDismissed);
				...
				PageDataGrid.ItemsSource = employees;
				ShowCountItemsPageDataGrid();

				var workMonth = EmployeesService.WorkMount();
				// Строковое название месяца года по указанному порядковому номеру
				var displayedMonth = CultureInfo.CurrentCulture.DateTimeFormat.GetMonthName(workMonth);
				WorkMonthLabel.Content = string.Format(workMonthPattern, displayedMonth);
			}
		...
		private void ShowCountItemsPageDataGrid()
		{
			...
			CountItemsLabel.Content = string.Format(countItemsPattern, PageDataGrid.Items.Count);
		}
		...
		{
			var employee = (Employee)rawEntity;
			...
			result &= !filter.GetValue("PersonnelNumber", out buffer) ||
			          FilterCriterias.ContainsLong(employee.PersonnelNumber, buffer);
			result &= !filter.GetValue("LastName", out buffer) ||
			          FilterCriterias.ContainsLine(employee.LastName, buffer);
			result &= !filter.GetValue("FirstName", out buffer) ||
			          FilterCriterias.ContainsLine(employee.FirstName, buffer);
			result &= !filter.GetValue("MiddleName", out buffer) ||
			          FilterCriterias.ContainsLine(employee.MiddleName, buffer);
			result &= !filter.GetValue("PassportSeriesAndNumber", out buffer) ||
			          FilterCriterias.ContainsLine(employee.PassportSeriesAndNumber, buffer);
			result &= !filter.GetValue("PassportIssuedByOrganization", out buffer) ||
			          FilterCriterias.ContainsLine(employee.PassportIssuedByOrganization, buffer);
			result &= !filter.GetValue("PassportIssueDate", out buffer) ||
			          FilterCriterias.ContainsDate(employee.PassportIssueDate, buffer);
			result &= !filter.GetValue("Profession", out buffer) ||
			          FilterCriterias.ContainsLine(employee.Profession, buffer);
			return result;
		}
